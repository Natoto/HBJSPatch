...
{
    [JPLoader updateToVersion:version ex:@"" callback:callback];
}
...
+ (void)updateToVersion:(NSInteger)version ex:(NSString *)ex callback:(JPUpdateCallback)callback{
    ...
    NSURL *downloadURL;
    if (ex.length) {
        downloadKey = [NSString stringWithFormat:@"%@", ex];
        downloadURL = [NSURL URLWithString:downloadKey];
    }
    else {
        downloadKey = [NSString stringWithFormat:@"/%@/v%@.zip%@", appVersion, @(version),ex];
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (!error) {
            ...
            if (!scriptZipFilePath) {
                scriptZipFilePath = downloadTmpPath;
            }
            
            // 2. decrypt and verify md5 file
            if (!isFailed && jp_md5verify) {
...
+ (void)clearVersion{
    NSString *appVersion = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleShortVersionString"];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:kJSPatchVersion(appVersion)];
}
